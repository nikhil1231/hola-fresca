...
def client(retailer: str):
    ...
    name, so this needs no per-retailer branch. Both shops are reached over
    plain HTTP; if one ever has to go back to being driven through a browser,
    this is where that choice belongs, not in the pipeline.
    """
    return get_adapter(retailer).Client()
