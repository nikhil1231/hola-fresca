...
@contextmanager
def preserve_after_personal_write(_session: Session):
    """Compatibility wrapper for personal writes.

    Personal tables are deliberately absent from the revision triggers, so
    their commits no longer change the planner signature. The surrounding APIs
    can keep marking the scope of those writes without serialising them behind
    the cache lock.
    """
    yield
...
def get_ranking(
    ...
    *,
    ...
    standalone_prices = get_standalone_prices(
        factory,
        servings=candidate_portions,
