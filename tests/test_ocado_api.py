"""The Ocado-only routes. The client is faked; nothing here reaches the network.

Sessions, login, OTP and the basket push moved to ``/api/cart/{retailer}`` and
are tested in tests/test_cart_api.py, for both shops. What is left here is what
only Ocado has: delivery slots.
"""
